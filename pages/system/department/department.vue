				...
				<view class="container">
					<view class="filter-container d-flex">
						<view class="filter-item d-flex">
							<button type="primary" size="mini" style="height: 35px;line-height: 35px;" @click="toPage('/pages/core/add', {'pid': 0, 'level': 1})">新增主部门</button>
						</view>
					</view>
							...
							<template v-if="module.table_json != null">
								<uni-td v-for="(table, tableIndex) in module.table_json" :key="tableIndex" :align="tableIndex==2?'left':'center'" v-if="table.show">
									<template v-if="tableIndex==2">
										...
										<view v-else-if="item.level == 4" style="margin-left: 30px;">|-{{ item[table.name] }}</view>
									</template>
									<template v-else>
								...
								<view class="d-flex">
									<template v-if="module.api_json != null">
										...
										<view class="tag-view" style="min-width: 60px">
											<uni-tag v-if="item.level==1" text="+子部门" type="primary" @click="toPage('/pages/core/add', {'pid': item.id, 'level': item.level+1})"></uni-tag>
											<uni-tag v-else-if="item.level==2" text="+子部门" type="warning" @click="toPage('/pages/core/add', {'pid': item.id, 'level': item.level+1})"></uni-tag>
											<uni-tag v-else-if="item.level==3" text="+子部门" type="default" @click="toPage('/pages/core/add', {'pid': item.id, 'level': item.level+1})"></uni-tag>
										</view>
									</template>
								</view>
							</uni-td>
